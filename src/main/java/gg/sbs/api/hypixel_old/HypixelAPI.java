...
import gg.sbs.api.http_old.HttpClient;
import gg.sbs.api.http_old.HttpParameter;
...
public class HypixelAPI {
	...
	private static final transient Gson GSON = new GsonBuilder()
			.registerTypeAdapter(HypixelProfile_old.Stats.SkyblockStats.class, new HypixelProfile_old.Stats.Deserializer())
	...
	private final transient ConcurrentSet<HypixelProfile_old> cache = Concurrent.newSet();
	...
	public final HypixelGuild_old getHypixelGuild(MojangProfile profile) throws HypixelApiException {
		...
		return GSON.fromJson(guildQuery, HypixelGuild_old.class);
	}

	public final void getHypixelGuild(MojangProfile profile, ResultCallback<HypixelGuild_old, HypixelApiException> callback) {
		Preconditions.checkArgument(callback != null, "Callback cannot be NULL!");

		Scheduler.getInstance().runAsync(() -> {
			HypixelGuild_old hypixelGuild = null;
	...
	public final HypixelProfile_old getHypixelProfile(MojangProfile profile) throws HypixelApiException {
		Preconditions.checkArgument(profile != null, "Profile cannot be NULL!");
		HypixelProfile_old hypixelProfile = null;

		// Remove Expired Cache Profiles
		this.cache.removeIf(HypixelProfile_old::hasExpired);

		// Check Cache Profiles
		if (!this.cache.isEmpty()) {
			for (HypixelProfile_old cachedProfile : this.cache) {
		...
		if (hypixelProfile == null) {
			...
			hypixelProfile = GSON.fromJson(playerQuery, HypixelProfile_old.class);
		...
		hypixelProfile.session = GSON.fromJson(statusQuery, HypixelProfile_old.Session.class);
		this.cache.add(hypixelProfile);
		return hypixelProfile;
	}

	public final void getHypixelProfile(MojangProfile profile, ResultCallback<HypixelProfile_old, HypixelApiException> callback) {
		Preconditions.checkArgument(callback != null, "Callback cannot be NULL!");
	...
	public final ConcurrentMap<String, Object> getSession(HypixelProfile_old hypixelProfile) throws HypixelApiException {
	...
	public final SkyblockIsland getSkyblockIsland(HypixelProfile_old hypixelProfile) throws HypixelApiException {
		return this.getSkyblockIsland(hypixelProfile, hypixelProfile.getFirstSkyblockProfile());
	}

	public final SkyblockIsland getSkyblockIsland(HypixelProfile_old hypixelProfile, HypixelProfile_old.SkyblockProfile skyblockProfile) throws HypixelApiException {
		Preconditions.checkArgument(hypixelProfile != null, "Hypixel profile cannot be NULL!");
	...
	public final ConcurrentSet<SkyblockIsland> getSkyblockIslands(HypixelProfile_old hypixelProfile) throws HypixelApiException {
		Preconditions.checkArgument(hypixelProfile != null, "Hypixel profile cannot be NULL!");
	...
	public final void getSkyblockIsland(HypixelProfile_old hypixelProfile, ResultCallback<SkyblockIsland, HypixelApiException> callback) {
		this.getSkyblockIsland(hypixelProfile, hypixelProfile.getFirstSkyblockProfile(), callback);
	}

	public final void getSkyblockIsland(HypixelProfile_old hypixelProfile, HypixelProfile_old.SkyblockProfile skyblockProfile, ResultCallback<SkyblockIsland, HypixelApiException> callback) throws HypixelApiException {
		Preconditions.checkArgument(callback != null, "Callback cannot be NULL!");
