...
public class DefaultParser implements CommandLineParser {
    ...
    public CommandLine parse(Options options, String[] arguments, Properties properties, boolean stopAtNonOption)
        ...
        cmd = Reflection.of(CommandLine.class).newInstance();
    ...
    private void handleToken(String token) throws ParseException {
        ...
        if (skipParsing)
            ...
            if (length > 1 && token.startsWith("\"") && token.endsWith("\"") && token.substring(1, length - 1).indexOf('"') == -1)
    ...
    private boolean isLongOption(String token) {
        ...
        if (!options.getMatchingOptions(t).isEmpty()) {
            ...
            return true;
        } else return getLongPrefix(token) != null && !token.startsWith("--");
    }
    ...
    private void handleShortAndLongOption(String token) throws ParseException {
        String t = StringUtil.stripStart(token, "-");
