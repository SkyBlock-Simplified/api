package dev.sbs.api.util.matcher;
...
public final class StringMatcherFactory {
    ...
    public StringMatcher andMatcher(final StringMatcher... stringMatchers) {
        final int len = ArrayUtil.getLength(stringMatchers);
        if (len == 0) {
            return NONE_MATCHER;
        }
        if (len == 1) {
    ...
    public StringMatcher charSetMatcher(final char... chars) {
        final int len = ArrayUtil.getLength(chars);
        if (len == 0) {
            return NONE_MATCHER;
        }
        if (len == 1) {
    ...
    public StringMatcher charSetMatcher(final String chars) {
        final int len = StringUtil.length(chars);
        if (len == 0) {
            return NONE_MATCHER;
        }
        if (len == 1) {
    ...
    public StringMatcher stringMatcher(final char... chars) {
        final int length = ArrayUtil.getLength(chars);
    ...
    public StringMatcher stringMatcher(final String str) {
        return StringUtil.isEmpty(str) ? NONE_MATCHER : stringMatcher(str.toCharArray());
    }
